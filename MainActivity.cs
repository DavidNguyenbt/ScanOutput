    ...
    public class MainActivity : Activity
    {
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            void Load()
            {
                ...
                {
                    Toast.MakeText(this, "Unable to connect to server or Language database invalid" + "\n" + ex.Message, ToastLength.Short).Show();
                }
            }
        }
        ...
        private void LoginSuccess(DataTable dtLoginDetail)
        {
            ...
            try
            {
                //if (!string.IsNullOrEmpty(WrkHrId))
                {
                    ...
                    DataTable dt = CSDL.Cnnt.Doc("select * from InlineWrkHrMaster where ShiftNm = '" + strSelectedFac + "'").Tables[0];
                    if (dt.Rows.Count > 0)
                    {
